...
public class MusicService {
	...
	public boolean hasMusic(String md5Value) {
		...
		return Jdbc.findOne(collection_music, eq("md5Value", md5Value), new TypeReference<MusicBean>() {
		}) == null ? false : true;
	}
	...
	public boolean remove(HttpServletRequest request) {
		...
		MusicSheetBean musicSheet = Jdbc .findOne(collection_musicsheet, eq("_id", musicsheetId), new TypeReference<MusicSheetBean>() {
		});
		if (musicSheet != null && musicSheet.getMusics().remove(musicId))
			return Jdbc .update(collection_musicsheet, musicSheet);
		else
			return false;
	}
	...
	public boolean transmusic(HttpServletRequest request) {
		...
		String musicSheetId=request.getParameter("MusicSheetId");
		MusicBean music=Jdbc .findOne(collection_music, eq("md5Value", md5Value), new TypeReference<MusicBean>() {});
		MusicSheetBean musicSheet = Jdbc .findOne(collection_musicsheet, eq("_id", musicSheetId), new TypeReference<MusicSheetBean>() {});
...
//                String fileName = value.substring(start+1);
		...
		Jdbc .insert(collection_music, music);
